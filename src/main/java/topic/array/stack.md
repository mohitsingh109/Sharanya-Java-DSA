...
- In Java, a stack can be implemented using:

  - Using a **Linked List** with a custom `Node` class
  - **Stack<E>** class (`java.util.Stack`) *(legacy, synchronized)*
  - **Deque<E>** with `ArrayDeque<E>` (**recommended**)
  - **LinkedList<E>** implementing the `Deque` interface

> **Recommendation:** Use `ArrayDeque` instead of `Stack` for better performance in modern Java.

---

# 2. Common Operations on Stack

| Operation | Java Code (`ArrayDeque`) | Average Time | Worst Case |
|-----------|--------------------------|--------------|------------|
| Push (insert) | `stack.push(x)` | O(1) | O(1) |
| Pop (remove) | `stack.pop()` | O(1) | O(1) |
| Peek (top) | `stack.peek()` | O(1) | O(1) |
| isEmpty | `stack.isEmpty()` | O(1) | O(1) |
| Size | `stack.size()` | O(1) | O(1) |
